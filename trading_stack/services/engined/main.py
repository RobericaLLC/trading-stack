...
@app.command()
def main(bars_path: str = "data/synth_bars.parquet", threshold: float = 0.5, max_notional: float = 2000, band_bps: int = 150):
    bars = read_events(bars_path, Bar1s)
    eng = DecisionEngine(symbol="SPY", threshold=threshold, max_notional=max_notional, price_band_bps=band_bps)
    intents = 0
    for b in bars:
        orders = eng.on_bar(b)
        intents += len(orders)
