...
def test_bar1s_creation() -> None:
    ...
    ts = datetime.now(UTC)
    bar = Bar1s(ts=ts, symbol="SPY", open=500.0, high=501.0, low=499.0, close=500.5, volume=100)
...
def test_market_trade_creation() -> None:
    ...
    ts = datetime.now(UTC)
    trade = MarketTrade(ts=ts, symbol="SPY", price=500.0, size=100, venue="NYSE")
...
def test_market_quote_creation() -> None:
    ...
    ts = datetime.now(UTC)
    quote = MarketQuote(ts=ts, symbol="SPY", bid=499.9, ask=500.1, bid_size=100, ask_size=200)
...
def test_new_order_creation() -> None:
    ...
    ts = datetime.now(UTC)
    order = NewOrder(
        symbol="SPY", side="BUY", qty=10.0, tif="DAY", limit=500.0, tag="test_order", ts=ts
    )
