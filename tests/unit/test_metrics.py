...
def test_freshness_and_gaps() -> None:
    trades = [
        MarketTrade(
            ts=_t(0), symbol="SPY", price=1.0, size=1,
            ingest_ts=_t(0) + timedelta(milliseconds=120)
        ),
        MarketTrade(
            ts=_t(1), symbol="SPY", price=1.0, size=1,
            ingest_ts=_t(1) + timedelta(milliseconds=80)
        ),
        MarketTrade(
            ts=_t(4), symbol="SPY", price=1.0, size=1,
            ingest_ts=_t(4) + timedelta(milliseconds=100)
        ),  # 3s gap (>2)
    ]
